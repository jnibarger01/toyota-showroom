 ...
 * (`Vehicle.pricing.currency` is `"USD"`). Display uses **`en-US`** via `Intl.NumberFormat` so
 * grouping and the `$` symbol stay consistent across builder financing, comparison, garage, and
 * option chips — independent of the browser's UI locale.
 *
...
function cacheKey(options: Required<Pick<FormatCurrencyOptions, "maximumFractionDigits" | "minimumFractionDigits" | "signed">>): string {
  return `${options.maximumFractionDigits}:${options.minimumFractionDigits}:${options.signed ? "s" : "u"}`;
}
...
function getCurrencyFormatter(options: FormatCurrencyOptions = {}): Intl.NumberFormat {
  ...
  const key = cacheKey({ maximumFractionDigits, minimumFractionDigits, signed });
...
/**
 * Format a USD amount with the showroom default locale (`en-US`).
 *
